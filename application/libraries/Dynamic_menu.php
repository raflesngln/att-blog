...
function build_menuuuuu()
    ...
    $group =  $CI->session->userdata('idgroup_usr');
	...
	$sq1 = "SELECT a.id_menu,a.title,a.url,a.parent,LENGTH(a.parent) AS par,a.icon,'1' as `show`,is_parent FROM dyn_menu a 
            ...
            WHERE LEFT(parent,1)='$parent' and parent <> '0' and id_group = '$group' ORDER BY b.id_menu "; 
...
function build_menu(){
	...
	$sq1 = "SELECT a.id_menu,a.title,a.url,a.parent,LENGTH(a.parent) AS par,a.icon,'1' as `show`,is_parent FROM dyn_menu a 
            INNER JOIN role_user b ON a.id_menu=b.id_menu 
            WHERE a.parent <> '0' and id_group = '$group' ORDER BY b.id_menu "; 
            // LEFT(parent,1)='$parent' and 
					...
					if ($menu[$xid]['is_parent'] == 1)
                    {
                        ...
                        $html_out .= '<li title="'.$menu[$xid]['title'].'">
                                        <a href="#">
											...
											<span class="nav-icon"><i class="material-icons">'.$row->icon.'</i></span>
											<span class="nav-text">'.$menu[$xid]['title'].'</span>  
                                        </a>';
                    }
                    else
                    {
                       // $html_out .= "\t\t\t\t".'<li>'.anchor($menu[$xid]['url'], '<i class="'.$menu[$xid]['icon'].'"></i> <span class="menu-text">'.$menu[$xid]['title'].'</span>');
                        $html_out .= '<li title="'.$menu[$xid]['title'].'">
                                        <a href="'.base_url().$menu[$xid]['url'].' " id="sub">
										    <span class="nav-caret"><i class="fa fa-caret-down"></i></span>
		...
		$xid = $tampung[$a];
            ...
            if ($menu[$xid ]['parent'] == $parent_id)    // are we allowed to see this menu?
            {
                ...
                if ($menu[$xid ]['is_parent'] == 1)
                {
                    $html_out .= "\t\t\t\t\t\t".'                  
						  <li>
						  <a href="inbox.html"><span id="ortu" class="nav-text">'.$menu[$xid]['title'].'</span></a>
                ...
                {
                    $html_out .= "
					\t\t\t\t\t\t".'<li><a href="'.base_url().$menu[$xid]['url'].'" id="childs"><span class="nav-text">'.$menu[$xid]['title'].'</span></a>';                    
                }
      ...
      // $html_out .= "\t\t".'<ul >' . "\n";
	   ...
	   for ($a = 1; $a <= count($menu); $a++)
		...
		//echo "<br />====================================>$xid<br />";
			if (is_array($menu[$xid]))    // must be by construction but let's keep the errors home
            {
                    $html_out .= '<a class="mngrup" href="'.base_url().$menu[$xid]['url'].'/'.$menu[$xid]['id'].'">
