...
class Inquiry extends CI_Controller{
    ...
    function index(){
		  ...
		  'scrumb'=>'<a href="'.base_url().'logistic/Inquiry" class="breadcrumb">Inquiry</a> Inquiry List',
		  'sidemenu'=>'<a href="'.base_url().'">Inquiry</a>',
		  'view'=>'logistic/inquiry/list_inquiry'
		  );
    ...
    function save_quotation(){
		  ...
		  'modulname'=>'CRM for Forwarder',
		  'scrumb'=>'<a href="'.base_url().'logistic/Inquiry" class="breadcrumb">Inquiry</a> Create Quotation',
		  'sidemenu'=>'<a href="'.base_url().'">Inquiry</a>',
		  'view'=>'logistic/inquiry/create_quotation'
		  );
	      $this->load->view('template/home',$data);
    ...
    function dashboard(){
		  ...
		  'Customer'=>$this->Model_inquiry->count_record("trcust_hdr","WHERE Status='Customer'"),
		  'aktive'=>$this->Model_inquiry->count_record("trcust_hdr","WHERE isActive='1'"),
		  'nonaktive'=>$this->Model_inquiry->count_record("trcust_hdr","WHERE isActive='0'"),
 ...
 function inquiry_detail(){
	 $idcust=$this->uri->segment(4);
		  ...
		  'attach'=>$this->Model_inquiry->getDataCustom("*","trcust_dtlphoto",""),
		  'modulname'=>'CRM for Forwarder',
		  'scrumb'=>'<a href="'.base_url().'logistic/Inquiry" class="breadcrumb">Inquiry</a> Inquiry Detail',
		  'idcust'=>$this->Model_inquiry->getDataCustom("a.Id as id_header,a.Name as companyname,b.Name as city,c.Name as country,a.Status,a.TotEmployee,a.Email,a.Website,a.isActive",
		  			"trcust_hdr a",
		  			"LEFT JOIN mscity b ON a.IdCity=b.Id
					 LEFT JOIN mscountry c ON a.IdCountry=c.Id
 ...
 function manage_inquiry(){
	 $idcust=$this->uri->segment(4);
       	  $data=array(
		  'title'=>'Quotation of Inquery ',
		  'modulname'=>'CRM for Forwarder',
		  'scrumb'=>'<a href="'.base_url().'logistic/Inquiry" class="breadcrumb">Inquiry</a> Quotation of Inquery ',
		  'idcust'=>$this->Model_inquiry->getDataCustom("a.Id as id_header,a.Name as companyname,b.Name as city,c.Name as country,a.Status,a.TotEmployee,a.Email,a.Website,a.isActive",
		  			"trcust_hdr a",
		  			"LEFT JOIN mscity b ON a.IdCity=b.Id
					 LEFT JOIN mscountry c ON a.IdCountry=c.Id
 ...
 function create_inquiry(){
	 $idcust=$this->uri->segment(4);
		  ...
		  'sidemenu'=>'<a href="'.base_url().'">customer_detail</a>',
		  'record'=>$this->Model_inquiry->getDataCustom("*","trcust_hdr"," WHERE Id='$idcust'"),
		  'view'=>'inquiry/create_quotation'
		  );
	      $this->load->view('template/home',$data);
	 
}
 function list_inquiry()
	{
		...
		$selected='a.*,b.Id as id_customer,b.Id as idcust,b.Status as status_app,b.Name as nm_customer,c.name as ori_country,d.Name as ori_city,e.Name as ori_port,f.name as dest_country,g.Name as dest_city,h.Name as dest_port,i.Name as nm_service,j.Desc as nm_commodity,l.Name as nm_ms_stage';
		...
		foreach ($list as $datalist){
            ...
            'nm_customer' =>'<a href="'.base_url().'customer/customer/customer_detail/'.$datalist->idcust.'">'.$datalist->nm_customer.'</a>',
			'ori_country' =>$datalist->ori_country,
			'ori_city' =>$datalist->ori_city,
			'ori_port' =>$datalist->IdPortOri,
			'dest_country' =>$datalist->dest_country,
			'dest_city' =>$datalist->dest_city,
			'dest_port' =>$datalist->IdPortDest,
			...
			'Status' =>$datalist->status_app=='Prospect'?'<span class="uk-badge uk-badge-warning">'.$datalist->status_app.'</span>':'<span class="uk-badge uk-badge-primary">'.$datalist->status_app.'</span>',
			...
			'action' =>'<div class="uk-button-dropdown" data-uk-dropdown>
                                ...
                                <div class="uk-dropdown uk-dropdown-small">
                                    <ul class="uk-nav uk-nav-dropdown">
                                        <li><a href="'.site_url('logistic/Inquiry/manage_inquiry').'"><i class="material-icons">description</i>Quotation</a></li>
                                        <li><a href="'.site_url('logistic/Inquiry/inquiry_detail/20161118INQUERY001').'"><i class="material-icons">settings</i>Detail</a></li>
                                    </ul>
                                </div>
                            </div>',
...
function status_inquiry()
	{
		...
		foreach ($list as $datalist){
            ...
            'id_header' =>$datalist->Id,
			'id_customer' =>$datalist->id_customer,
            'nm_customer' =>'<a href="'.base_url().'logistic/inquiry/detail_inquiry/'.$datalist->Id.'">'.$datalist->nm_customer.'</a>',
			'ori_country' =>$datalist->ori_country,
			'ori_city' =>$datalist->ori_city,
