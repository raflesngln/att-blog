...
class Ticket extends CI_Controller{
 ...
 function customer_detail(){
	 $idcust=$this->uri->segment(4);
		  ...
		  'idcust'=>$this->Crm_model->getDataCustom("a.Id as id_header,a.Name as companyname,b.Name as city,c.Name as country,a.Status,a.TotEmployee,a.Email,a.Website,a.isActive",
		  			"trcust_hdr a",
		  ...
		  'record'=>$this->Crm_model->getDataCustom("*","trcust_hdr"," WHERE Id='$idcust'"),
...
//	 		print_r($data);
 ...
 function list_customer()
	{
		...
		$where=  array();
        $list = $this->Crm_model->get_datatables2($selected,$nm_tabel,$nm_coloum,$orderby,$where,$nm_tabel2,$kolom1,$kolom2);
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->Crm_model->count_all2($nm_tabel,$nm_coloum,$nm_tabel2,$kolom1,$kolom2),
						"recordsFiltered" => $this->Crm_model->count_filtered2($nm_tabel,$nm_coloum,$orderby,$where,$nm_tabel2,$kolom1,$kolom2),
						"data" => $data,
				);
 ...
 function reloadBoxContent(){
	 ...
	 if($mydata=='general'){
		$search=$this->Crm_model->getDataCustom("*","trcust_hdr"," WHERE Id='$myid'");  
	 } else if($mydata=='address'){
		 $search=$this->Crm_model->getDataCustom("a.Id as id_detail,a.IdAddrType,a.IdCustHeader,a.Address,a.Postal,a.Telp1,a.Telp2,a.Fax,a.isDefault,a.Remarks,b.Id as id_type,b.Name as nm_type",
		 		 "trcust_dtladdress a",
				 "LEFT JOIN msaddr_type b ON a.IdAddrType=b.Id 
				  WHERE IdCustHeader='$myid'");
	 } else if($mydata=='sales'){
		 $search=$this->Crm_model->getDataCustom("a.*,b.Id as id_service,b.Name as nm_sevice","trcust_dtlsales a",
		 ...
		 $search=$this->Crm_model->getDataCustom("a.*,b.Description as nm_business_type,c.Name as nm_customer",
		 ...
		 $search=$this->Crm_model->getDataCustom("a.*,b.Desc as nm_commodity,c.Name as nm_service",
		 		 "trcust_dtlcomm a",
		 ...
		 $data=array(
		 	'record'=>$search,
			'type'=>$this->Crm_model->getDataCustom("*","msaddr_type","ORDER BY Name"),
...
/*        $otpt= $this->session->userdata('otopt');
       ...
       */
        ...
        $sql =$this->Crm_model->getDataCustom("*","mscountry a"," WHERE name like '%$kode%' ");
        $data = array();
       //$ii = 0;
...
function getCity(){
        $kode = $this->input->post('q');
        $sql =$this->Crm_model->getDataCustom("*","mscity a"," WHERE name like '%$kode%' ");
        $data = array();      
        foreach($sql as $row)
        {   
            $data[] = array(
	...
	public function saveGeneral()
	{
		...
		$this->Crm_model->update(array($key => $this->input->post('idgeneral')), $data,$nmtabel);
...
public function saveAddressType()
	{   
		...
		$insert = $this->Crm_model->save($data,$nmtabel);
		echo json_encode(array("status" => TRUE));
	}
function editAddress(){
	 $kode=$this->input->post('iddetail');
      $result=$this->Crm_model->getDataCustom("a.*,b.id as id_type,b.Name as nm_type","trcust_dtladdress a",
		  " LEFT JOIN msaddr_type b on a.IdAddrType=b.Id 
...
public function UpdateEditAddress()
	{
		...
		$this->Crm_model->update(array($key => $this->input->post('idaddress1')), $data,$nmtabel);
		echo json_encode(array("status" => TRUE));
	}
...
/*		if(isset($_POST['isdefault1'])){
		...
		$this->Crm_model->update(array($key => $this->input->post('idSales1')), $data,$nmtabel);
		echo json_encode(array("status" => TRUE));
	}
public function UpdateDetailCommodity()
	{
		...
		$this->Crm_model->update(array($key => $this->input->post('idCommodity1')), $data,$nmtabel);
		echo json_encode(array("status" => TRUE));
	}
public function deleteData()
	{
       ...
       $key    = $this->input->post('kolom');
		...
		$this->Crm_model->delete_by_id($id,$nmtabel,$key);
		echo json_encode(array("status" => TRUE));
	}
function editDetailSales(){
	 $kode=$this->input->post('id_detail');
      $result=$this->Crm_model->getDataCustom("a.*,b.Id as id_service,b.Name as nm_sevice","trcust_dtlsales a",
		  "LEFT JOIN msservice b ON a.IdService=b.Id
				   WHERE a.Id='$kode'");
	foreach($result as $list){
		$row = array(
...
function editDetailCommodity(){
	 $kode=$this->input->post('id_detail');
      $result=$this->Crm_model->getDataCustom("a.*,b.Desc as nm_commodity,c.Name as nm_service",
		 		 "trcust_dtlcomm a",
				 "LEFT JOIN mscommodity b ON a.IdComm=b.Id 
				  LEFT JOIN msservice c ON b.IdService=c.Id 
				  WHERE a.Id='$kode'");
...
function load_combo_service(){
      ...
      $result=$this->Crm_model->getDataCustom("a.*,b.Name as nm_kategori","msservice a",
...
function load_combo_commodity(){
      ...
      $result=$this->Crm_model->getDataCustom("a.*,b.Name as nm_service","mscommodity a",
		  "LEFT JOIN msservice b ON a.IdService=b.Id");
	foreach($result as $list){
		$row = array(
				'Id' =>$list->Id,
